...
def main(ticker, start, end, horizon=30, seq_len=30, epochs=100):
    ...
    if len(X) == 0:
        ...
        for h in range(horizon):
            try:
                print(f"Debug - Feature shapes: cur_res_seq: {cur_res_seq.shape}, cur_features: {cur_features.shape}")
                # Prepare feature array with proper structure (matching training structure)
                # [Residual, OHLC prices (4), Volume (1), Technical features (rest)]
                current_sequence = np.column_stack([
                    cur_res_seq.reshape(-1, 1),  # residual
                    cur_features[:, :4],  # OHLC
                    cur_features[:, 4:5],  # Volume
                    cur_features[:, 5:]  # Technical features
                ])
                print(f"Debug - current_sequence shape: {current_sequence.shape}")
                
                # Scale features
                current_sequence_scaled = np.zeros((1, seq_len, current_sequence.shape[1]))
                
                # Scale residual
                current_sequence_scaled[0, :, 0:1] = y_scaler.transform(np.log1p(np.abs(current_sequence[:, 0:1])))
                
                # Scale OHLC prices
                price_data = np.log1p(np.abs(current_sequence[:, 1:5]))
                current_sequence_scaled[0, :, 1:5] = price_scaler.transform(price_data)
                
                # Scale volume
                vol_data = np.log1p(np.abs(current_sequence[:, 5:6]))
                current_sequence_scaled[0, :, 5:6] = volume_scaler.transform(vol_data)
                
                # Scale technical features
                if current_sequence.shape[1] > 6:  # If we have technical features
                    tech_data = current_sequence[:, 6:]
                    current_sequence_scaled[0, :, 6:] = tech_scaler.transform(tech_data)
                
                # Make prediction
                pred_scaled = model.predict(current_sequence_scaled, verbose=0)[0]
